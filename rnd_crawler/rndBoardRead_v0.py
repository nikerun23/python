...
def print_RnD(csv_info, yesterday_list, keyword_list, wc_company_dict):
    ...
    for tr in board_list:
        try:
            ...
            # if util.yesterday_check(yesterday_list, board_date):
            if util.get_keyword_title(title, keyword_list):
                if 'onclick' == content_url:  # 제목 링크에 onclick 존재할 경우만
            ...
            #         title_href = href[href.find("'")+1:href.rfind("'")]
            #         logger.debug(content_url)
        except Exception as e:
            logger.error(e)
...
if __name__ == '__main__':
    ...
    try:
        ...
        # yesterday_list = [datetime.date(2018, 6, 12),datetime.date(2018, 6, 13)]
        ...
        print_list(56,56,999)  # 인자로 rowNum을 주면 제외하고 크롤링
